...
class HashMap:
    ...
    def __init__(self, capacity, function):
        ...
        self.size = 0
        self.num_empty_buckets = capacity
    ...
    def clear(self):
        ...
        for i in range(self.capacity):
            if self.empty_buckets is not None:
                ll = self._buckets[i]
                ll.head = None  # deleting the list
                ll.size = 0
        self.num_empty_buckets = 0
        HashMap.size = 0
    ...
    def resize_table(self, capacity):
        """
        Resize's the hash table to have a number of buckets equal to the given
        capacity. All links need to be rehashed in this function after resizing
        Args:
            capacity: the new number of buckets.
        """
        ht_new = HashMap(capacity, self._hash_function)

        for i in range(self.capacity):
            ll = self._buckets[i] # previous ll
            cur = ll.head
            while cur is not None:
                ht_new.put(cur.key, cur.value)
                cur = cur.next
        self.capacity = capacity
        self._buckets = ht_new._buckets
        self.num_empty_buckets = ht_new.num_empty_buckets
